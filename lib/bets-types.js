...
var getBetLAQU = function(err, window, agent, bets){
    ...
    $('div.contdepordos span.resuldeportab').each(function(index){
        ...
        var goals = $('div.contdepordos span.resuldeportabtres').eq(index).text().deleteBlankSpaces().split('-');
        var result = $(this).text();
        matchs.push({team1:team1, team2:team2, goals1:goals[0], goals2:goals[1], result:result});
    ...
    for (var i = 0, len = res.length; i < len; i+=5) {
        ...
        if ($(res[i+2]).attr("class") == "resuldepormarcador"){
            var goals = $(res[i+2]).text().deleteBlankSpaces().split('-'); 
            matchs.push({team1:team1, team2:team2, goals1:goals[0], goals2:goals[1]});
            ...
            matchs.push({team1:team1, team2:team2, date:date});
        }   
    }
